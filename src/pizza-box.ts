...
function buildOnChainMetadata(data: any): Cell {
  ...
  Object.entries(data).forEach(([key, value]) => {
    if (typeof value === 'number') {
      dict.set(
        toKey(key),
        beginCell().storeUint(0, 8).storeUint(value, 8).endCell(),
      );
    } else {
      dict.set(
        toKey(key),
        beginCell()
          .storeUint(0, 8)
          .storeStringTail(value as string)
          .endCell(),
      );
    }
  });
...
class PizzaBox {
  private static readonly lockerFactory = address(
    'EQA2apWqdiZiyJhnYDc-kjEWlMprUurkqqRfDQ3Sx-gdcImu',
  );
  ...
  public static async getLockTxParams(
    ...
    if (
      lockProps.firstUnlockPercent > 100 ||
      lockProps.firstUnlockPercent < 0
    ) {
